...
##
## Proot-Distro is a script for managing proot containers.
## Ported from termux/proot-distro for standalone Android use.
##
...
_get_distro_name() { eval "printf '%s' \"\${SUPPORTED_DISTRIBUTIONS__${1}}\""; }
_get_distro_comment() { eval "printf '%s' \"\${SUPPORTED_DISTRIBUTIONS_COMMENTS__${1}}\""; }
_has_distro() { eval "[ -n \"\${SUPPORTED_DISTRIBUTIONS__${1}+x}\" ]"; }
_has_distro_comment() { eval "[ -n \"\${SUPPORTED_DISTRIBUTIONS_COMMENTS__${1}+x}\" ]"; }
_set_distro_name() {
	eval "SUPPORTED_DISTRIBUTIONS__${1}=\"\${2}\""
	case " $_DISTRO_KEYS " in
		*" $1 "*) ;;
		*) _DISTRO_KEYS="$_DISTRO_KEYS $1" ;;
	esac
}
_set_distro_comment() { eval "SUPPORTED_DISTRIBUTIONS_COMMENTS__${1}=\"\${2}\""; }
_get_tarball_url() { eval "printf '%s' \"\${TARBALL_URL_${1}}\""; }
_get_tarball_sha256() { eval "printf '%s' \"\${TARBALL_SHA256_${1}}\""; }
_set_tarball_url() { eval "TARBALL_URL_${1}=\"\${2}\""; }
_set_tarball_sha256() { eval "TARBALL_SHA256_${1}=\"\${2}\""; }
...
command_install() {
	...
	if ! _has_distro "$distro_name"; then
		msg
	...
	if [ -n "${override_alias-}" ]; then
		if [ ! -e "${DISTRO_PLUGINS_DIR}/${override_alias}.sh" ] && [ ! -e "${DISTRO_PLUGINS_DIR}/${override_alias}.override.sh" ]; then
			...
			sed -i "s/^\(DISTRO_NAME=\)\(.*\)\$/\1\"$(_get_distro_name "$distro_name") - ${override_alias}\"/g" "${distro_plugin_script}"
			_set_distro_name "${override_alias}" "$(_get_distro_name "$distro_name")"
	...
	if [ -f "${distro_plugin_script}" ]; then
		msg "${BLUE}[${GREEN}*${BLUE}] ${CYAN}Installing ${YELLOW}$(_get_distro_name "$distro_name")${CYAN}...${RST}"
		...
		_set_tarball_url aarch64 ""
		_set_tarball_url arm ""
		_set_tarball_url i686 ""
		_set_tarball_url riscv64 ""
		_set_tarball_url x86_64 ""
		_set_tarball_sha256 aarch64 ""
		_set_tarball_sha256 arm ""
		_set_tarball_sha256 i686 ""
		_set_tarball_sha256 riscv64 ""
		_set_tarball_sha256 x86_64 ""
		...
		if [ -n "${PD_OVERRIDE_TARBALL_URL-}" ]; then
			_set_tarball_url "$DISTRO_ARCH" "${PD_OVERRIDE_TARBALL_URL}"
			_set_tarball_sha256 "$DISTRO_ARCH" "${PD_OVERRIDE_TARBALL_SHA256}"
		fi
		...
		if [ -z "$(_get_tarball_url "$DISTRO_ARCH")" ]; then
		...
		if [ -z "${PD_OVERRIDE_TARBALL_URL-}" ] && [ -n "${PD_OVERRIDE_TARBALL_SHA256-}" ]; then
			if ! grep -qE '^[0-9a-fA-F]{64}$' <<< "$(_get_tarball_sha256 "$DISTRO_ARCH")"; then
				msg
		...
		local archive_name
		archive_name=$(basename "$(_get_tarball_url "$DISTRO_ARCH")")

		if [ ! -f "${DOWNLOAD_CACHE_DIR}/${archive_name}" ]; then
			msg "${BLUE}[${GREEN}*${BLUE}] ${CYAN}Downloading rootfs archive...${RST}"
			msg "${BLUE}[${GREEN}*${BLUE}] ${CYAN}URL: $(_get_tarball_url "$DISTRO_ARCH")${RST}"
			msg

			if ! download_file "$(_get_tarball_url "$DISTRO_ARCH")" "${DOWNLOAD_CACHE_DIR}/${archive_name}" 3; then
				msg
		...
		if [ -n "$(_get_tarball_sha256 "$DISTRO_ARCH")" ]; then
			...
			if [ "$(_get_tarball_sha256 "$DISTRO_ARCH")" != "${actual_sha256}" ]; then
...
command_remove() {
	...
	msg "${BLUE}[${GREEN}*${BLUE}] ${CYAN}Wiping the rootfs of ${YELLOW}$(_get_distro_name "$distro_name")${CYAN}...${RST}"
...
command_rename() {
	...
	if ! _has_distro "$orig_distro_name"; then
		msg
	...
	if [ -e "${DISTRO_PLUGINS_DIR}/${orig_distro_name}.override.sh" ]; then
		...
		sed -i "s/^\(DISTRO_NAME=\)\(.*\)\$/\1\"$(_get_distro_name "$orig_distro_name") - ${new_distro_name}\"/g" "${DISTRO_PLUGINS_DIR}/${new_distro_name}.override.sh"
	else
		msg
...
command_login() {
	...
	j=0
	for i in $fds; do
		realpath -e "/proc/self/fd/$j" >/dev/null 2>&1 && set -- "--bind=/proc/self/fd/$j:/dev/${i}" "$@"
		j=$((j + 1))
	done
	unset i j fds
...
command_list() {
	...
	if [ -z "$_DISTRO_KEYS" ]; then
		...
		local i
		for i in $(echo "$_DISTRO_KEYS" | tr ' ' '\n' | sort -d); do
			[ -z "$i" ] && continue
			if $verbose; then
				msg
				msg "  ${CYAN}* ${YELLOW}$(_get_distro_name "$i")${RST}"
				msg
				...
				if _has_distro_comment "$i"; then
					msg "    ${CYAN}Comment: $(_get_distro_comment "$i")${RST}"
				fi

				local supported_cpus
				if [ -f "${DISTRO_PLUGINS_DIR}/${i}.sh" ]; then
					supported_cpus=$(grep -oE 'TARBALL_URL_[a-zA-Z0-9_]+=' "${DISTRO_PLUGINS_DIR}/${i}.sh" 2>/dev/null | sed 's/TARBALL_URL_//;s/=//' | tr '\n' ' ')
				elif [ -f "${DISTRO_PLUGINS_DIR}/${i}.override.sh" ]; then
					supported_cpus=$(grep -oE 'TARBALL_URL_[a-zA-Z0-9_]+=' "${DISTRO_PLUGINS_DIR}/${i}.override.sh" 2>/dev/null | sed 's/TARBALL_URL_//;s/=//' | tr '\n' ' ')
				else
				...
				msg "  ${CYAN}* ${YELLOW}$(_get_distro_name "$i") ${GREEN}< $i >${RST}"
			fi
		done
...
command_backup() {
	...
	msg "${BLUE}[${GREEN}*${BLUE}] ${CYAN}Backing up ${YELLOW}$(_get_distro_name "$distro_name")${CYAN}...${RST}"
